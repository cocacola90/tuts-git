 ...
 **/
	public function buildExpressCheckoutNvp($order) {
		...
		$nvps = array(
			...
			//'PAYMENTREQUEST_0_SHIPPINGAMT' => $order['shipping']
		);
		...
		if (isset($order['items']) && is_array($order['items'])) {
			...
			if(isset($order['shippingDetail']))
			{
				...
				$nvps["ADDRESSOVERRIDE"] = 1;
				//$nvps["PAYMENTREQUEST_0_SHIPTONAME"] = $shippingDetail['firstname'] . ' ' . $shippingDetail['lastname'];
			    $nvps["L_PAYMENTREQUEST_FIRSTNAME"] = $shippingDetail['firstname'];
				 $nvps["L_PAYMENTREQUEST_LASTNAME"] =  $shippingDetail['lastname'];
				$nvps["PAYMENTREQUEST_0_SHIPTOSTREET"] = $shippingDetail['address'];
				if(!empty($shippingDetail['address2']))
				{
					$nvps["PAYMENTREQUEST_0_SHIPTOSTREET2"] = $shippingDetail['address2'];
				}
				
				$nvps["PAYMENTREQUEST_0_SHIPTOCITY"] = $shippingDetail['city'];
				$nvps["PAYMENTREQUEST_0_SHIPTOSTATE"] = $shippingDetail['state'];
				$nvps["PAYMENTREQUEST_0_SHIPTOZIP"] = $shippingDetail['zipcode'];
				$nvps["PAYMENTREQUEST_0_SHIPTOCOUNTRYCODE"] = $shippingDetail['code'];
				$nvps["PAYMENTREQUEST_0_SHIPTOCOUNTRYNAME"] = "United States";
				if(!empty($shippingDetail['tel'])){
					$nvps["PAYMENTREQUEST_0_SHIPTOPHONENUM"] = $shippingDetail['tel'];
				}
			}
