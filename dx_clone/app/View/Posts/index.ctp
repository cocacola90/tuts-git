...
<?php foreach($posts as $item):?>
	<article class="river-item promo-normal" id="river-item">
		<div class="media-head l-item-pull clearfix">
		<?php 
		$title = $item['Post']['title'];
		$image = $this->Html->image($this->Tool->get_thumbs($item['Post']['thumbnail']),array('alt'=> $title,'width'=>'220', 'height'=>'150'));
		echo $this->Html->link(
			$image,array(
				'controller' => 'posts',
				'action' => 'view',
		...
		<div class="media-body">
			...
			<p class="media-deck hidden-xs"><?php echo $this->Tool->substr($item['Post']['description'],0,100);?></p>
			<footer class="media-meta">
				<time class="media-date"><?php echo date('m/d/Y H:i' ,$item['Post']['created']);?></time>
			</footer>
		</div>
...
<?php endif;?>
	...
	<!------------------------------------------>
	<style type="text/css">
	#river-item{margin-bottom: 25px; }
	.media-head{float: left; padding-right: 10px;}
	.media-body{}
	.media-body h3.media-title{font-size: 24px; line-height: 30px;margin: 0px!important;}
	.media-deck{ margin: 10px 5px 5px 4px!important;}
	.media-date{font-size: 14px;color: #999;}
	@media only screen and (max-width: 768px) {
		.media-head{clear: both; float: none;}
