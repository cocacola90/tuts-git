(function($) {
	jQuery.fn.tooltip = function(options){
		 var defaults = {  
		    ...
		    bordercolor: '#cc0000', // tooltip border color
		    bgcolor: '#fcfcfc', //Tooltip background color
		    ...
		    height: 'auto', // Tooltip's Height
		    width : '280', //Tooltip's Width
		...
		$(this).hover(function(e){
				...
				if($(document).width() / 2 < e.pageX){
					$(dialog_id).css({
						...
						'left' :  options.loi_width,//e.pageX - $(dialog_id).width() + options.offsetX,
					...
					$(dialog_id).css({
						...
						'padding': '15px',
						'top' : e.pageY + options.offsetY,
						'left' : options.loi_width,//e.pageX + options.offsetX,
...
//alert(size);
				...
				if($(document).width() / 2 < e.pageX){		
					$(dialog_id).css({
						'top' : e.pageY + options.offsetY,
						'left' : options.loi_width,//e.pageX - $(dialog_id).width() - 40,
						'height' : options.height,
						'width' : options.width
					});
