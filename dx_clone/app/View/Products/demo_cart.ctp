...
<section class="breadcrumb-box">
    <ul class="breadcrumb">
        <li itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb"><a href="http://localhost/the-gioi-cong-nghe/"
                                                                             title="Trang chủ" itemprop="url"><span
                    itemprop="title"><img src="img/icon-homeb.png" title="Trang chủ" alt="Trang chủ"></span></a></li>
        <li class="separator">»</li>
        <li itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb"><h1><a href="gio-hang.html" title="Giỏ hàng"
...
<section class="cart">
    <?php echo $this->Form->create('Product', array('url' =>
        '/products/update_once')); ?>
    <table border="1">
        <thead>
        <tr>
            <th width="80">Ảnh</th>
        ...
        <?php foreach ($cart as $item): ?>
            <?php foreach ($item as $key => $value): ?>
                <?php $data = $this->requestAction(array(
                    'controller' => 'products',
                    'action' => 'get_info',
                    $value['slug'])); ?>
                ...
                <tr>
                    <td align="center">
                        <?php
                        echo $this->Html->link($this->Html->image($data['Product']['thumbnail'], array(
                            'class' => 'img-cart',
                            'width' => '80',
                            'height' => '80')), array(
                            'controller' => 'products',
                            ...
                            'id' => $data['Product']['id'],
                            'ext' => 'html'), array('escape' => false, 'class' => ''));
                        ?>

                    </td>
                    <td>
                        <?php
                        echo $this->Html->link($data['Product']['name'], array(
                            'controller' => 'products',
                            'action' => 'view_product',
                            'category' => $data['Category']['slug'],
                            'product' => $data['Product']['slug'],
                            'id' => $data['Product']['id'],
                            'ext' => 'html'), array('title' => $data['Product']['name'], 'class' =>
                            'name-cart'));
                        ?>
                    </td>
                    ...
                    <td align="right">
                        <?php if ($data['Product']['discount_status'] == 1): ?>
                            <?php echo $this->Tool->price($data['Product']['price'], $to_currency,
                    ...
                    <td align="center">
                        <input type="number" onkeyup="Check(this)"
                               name="product_<?php echo $data['Product']['id']; ?>_<?php echo
                               $value['key']; ?>"
                               id="product_<?php echo $data['Product']['id']; ?>" value="<?php echo
                        $value['quantity']; ?>"
                        ...
                        <?php if (isset($data['Product']['price'])) {
                            if ($data['Product']['discount_status'] == 1) {
                                ...
                                echo $this->Tool->number_format($this->Tool->price($data['Product']['price'], $to_currency,
                                    $data['Product']['discount'], $value['quantity']));
                            } else {
                                echo $this->Tool->number_format($this->Tool->price($data['Product']['price'], $to_currency,
                                    '0', $value['quantity']));
                    ...
                    <td align="center">
                        <a title="Xóa"
                           href="javascript:redirectUrl('<?php echo $this->Html->url('/products/delete_once/' .
                               $data['Product']['id'] . '/' . $value['key']); ?>','Bạn muốn xóa sản phẩm này không?')">
                            <section class="track">
                ...
                <?php if ($data['Product']['discount_status'] == 1): ?>
                    <?php $total += $this->Tool->get_price($data['Product']['price'], $data['Product']['discount']) *
                        $value['quantity']; ?>
        ...
        <tr>
            ...
            <td>  <?php echo $this->Form->input('Cập
                nhật', array('type' => 'submit', 'class' => 'update-cart',
                    'label' => false)); ?>
            </td>
            <td></td>
        </tr>
            ...
            <td>
                <select id="shipping_to" name="">
                    ...
                    <?php foreach ($country as $item): ?>
                        <option value="<?php echo $item['Country']['code']; ?>"><?php echo
                            $item['Country']['country']; ?></option>
                    <?php endforeach; ?>
                </select>
        ...
        <tr>
            ...
            <td>
                <a class="buy-next-cart" href="/thanh-toan" title="Thanh toán">Thanh toán</a>
            </td>

            <td align="center">
                <a class="del-cart"
                   href="javascript:redirectUrl('<?php echo $this->Html->url(array
                   ('controller' => 'products', 'action' => 'delete_all')); ?>','Bạn muốn xóa toàn bộ giỏ hàng?')"
...
<section class="payment">
    ...
    <?php echo $this->Form->input('user_id', array(
        ...
        'label' => false,
        'div' => false)); ?>
    <?php echo $this->Form->input('ship_type', array('type' => 'hidden', 'id' =>
        'type_ship')); ?>
    <?php echo $this->Form->input('code', array('type' => 'hidden', 'id' =>
        'ship_to')); ?>
    <?php echo $this->Form->input('total_weight', array('type' => 'hidden', 'id' =>
        'weight')); ?>
    <table>
        <tbody>
        <tr>
        ...
        <tr>
            ...
            <td>
                <?php echo $this->Form->textarea('note', array('class' =>
                    'area-payment')); ?>
            </td>
        </tr>
        <tr>
            ...
            <td>
                ...
                hàng', array('class' => 'btn-payment', 'type' => 'submit',
                    'label' => false)); ?>
    ...
    $('#control_ship').change(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                data = ceil(data);
                var format_data = new Intl.NumberFormat('de-DE', {
                    style: 'currency',
                ...
                var format_grand_total = new Intl.NumberFormat('de-DE', {
                    style: 'currency',
                    currency: to_currency
                }).format(grand_total);
